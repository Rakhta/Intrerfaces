...
void Lab3::buttonClicked()
{
    ...
    QProcess qqq;            //mutex - базовый элемент синхронизации, предназначенный для организации доступа к общим
    QMutex mutex;            //данным для нескольких потоков.
    mutex.lock();            // mutex.lock - захват мьютекса(поток запрашивает использование общих данных)
    mutex.tryLock(1000);     // mutex.unlock - освобождение мьютекса
    mutex.unlock();
    qqq.start("xset q");
    qqq.waitForFinished();
